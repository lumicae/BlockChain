...
//        @PostConstruct
        public void runTests() throws Exception {
            String cowAcct = jsonRpc.personal_newAccount("cow");//cowAcct:0xcd2a3d9f938e13cd947ec05abc7fe734df8dd826
            String bal0 = jsonRpc.eth_getBalance(cowAcct);
            System.out.println("Balance: " + bal0);//0x100000000000000000000000000000000000000000000000000
            assertTrue(TypeConverter.StringHexToBigInteger(bal0).compareTo(BigInteger.ZERO) > 0);

            String pendingTxFilterId = jsonRpc.eth_newPendingTransactionFilter();//0x1
            ...
            String bal1 = jsonRpc.eth_getBalance(cowAcct);//0xffffffffffffffffffffffffffffffffffffadf7ffffff8889
        ...
        String mineBlock() throws InterruptedException {
            ...
            while (true) {
                ...
                cnt += blocks.length;
                if (cnt > 0) {
                    hash1 = (String) blocks[0];//循环了好久，才跳到此处
                    break;
                }
                Thread.sleep(100);
            }
